...
namespace UnityTutorial.PlayerControl
{
    public class PlayerController : MonoBehaviour
    {
        ...
        private void HandleJump()
        {
            ...
            // Increase the force for a higher jump
            _playerRigidbody.AddForce(Vector3.up * JumpFactor, ForceMode.Impulse);

            // Reset the jump trigger after the jump
            _animator.ResetTrigger(_jumpHash);

            // Add a function call to reset jump state
            ResetJumpState();
        }
        private void ResetJumpState()
        {
            _grounded = false; // Assume the player is not grounded immediately after jumping
            StartCoroutine(CheckGrounded()); // Start a coroutine to check for grounded state
        }
